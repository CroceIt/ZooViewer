...
public class App {
	...
	public static void main(String[] args) throws IOException {
		...
		DefaultTreeCellRenderer renderer = new DefaultTreeCellRenderer() {
			 ...
			 */
			private static final long serialVersionUID = 8950248718728508224L;
	...
	private static String inputConnectionString(String defaultString) {
		JOptionPane pane = new JOptionPane(bundle.getString("start.connection.message"), JOptionPane.QUESTION_MESSAGE, JOptionPane.OK_CANCEL_OPTION);
		pane.setWantsInput(true);
		pane.setInputValue(defaultString);
		pane.setInitialValue(defaultString);
		pane.setInitialSelectionValue(defaultString);
		JDialog dialog = pane.createDialog(null, bundle.getString("start.connection.title"));
	...
	private static void disposeOtherWindows(final JFrame jf, final JFrame jf2) {
		...
		for (Window window : windows) {
			if (!window.getName().equals(jf.getTitle()) && !window.getName().equals(jf2.getTitle())) {
				window.dispose();
			}
		}
	}
	...
	private static JZVTree createViewOnlyTree(final ZVModel model) {
		...
		tree2.getModel().addTreeModelListener(new TreeModelAdapter() {
			...
			public void treeNodesChanged(TreeModelEvent e) {
				System.out.println("App.main(...).new TreeModelAdapter() {...}.treeNodesChanged()");
				final TreePath childPath = e.getTreePath().pathByAddingChild(e.getChildren()[0]);
				this.selectAndDisplayPath(tree2, childPath);
			}

			@Override
			public void treeNodesInserted(TreeModelEvent e) {
				System.out.println("App.main(...).new TreeModelAdapter() {...}.treeNodesInserted()");
				Object[] children = e.getChildren();
				final TreePath lastChildPath = e.getTreePath().pathByAddingChild(children[children.length - 1]);
				this.selectAndDisplayPath(tree2, lastChildPath);
			}

			@Override
			public void treeNodesRemoved(TreeModelEvent e) {
				System.out.println("App.main(...).new TreeModelAdapter() {...}.treeNodesRemoved()");
				this.selectAndDisplayPath(tree2, e.getTreePath());
			}

			private void selectAndDisplayPath(final JZVTree tree2, final TreePath childPath) {
				SwingUtilities.invokeLater(new Runnable() {

					@Override
					public void run() {
						TreeSelectionModel selectionModel = tree2.getSelectionModel();
